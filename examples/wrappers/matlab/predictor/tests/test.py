...
from examples.wrappers.matlab.predictor.matlab_kalman import\
    MatlabKalmanPredictor
...
def test_kalman():
    ...
    eval_prediction = GaussianStatePrediction(
        ...
        cv.matrix(timestamp=new_timestamp,
                  time_interval=time_interval)@prior.covar
        @cv.matrix(timestamp=new_timestamp,
    ...
    kp = MatlabKalmanPredictor(transition_model=cv,
                               dir_path="./")
