...
for mc in range(0,MC):
    ...
    priorUKF = GaussianState([ [10000],[75],[30000],[0]], np.diag([120, 20, 120, 20]), timestamp=start_time)
...
axs[0].set_xticklabels(['GMF', 'PMF', 'Sys', 'Res', 'Strat'])
...
axs[1].set_xticklabels(['GMF', 'PMF', 'Sys', 'Res', 'Strat'])
...
axs[2].set_xticklabels(['GMF', 'PMF', 'Sys', 'Res', 'Strat'])
axs[2].set_ylabel(r'\textbf{SNEES} Position')
axs[2].set_ylim([0, 3])

fig.savefig("STATS_HIGH_HILLY.pdf", format='pdf', dpi=1000, bbox_inches='tight')
...
mean_times = [mean_time_GMF, mean_time_PMF, mean_time_Strat, mean_time_Residual, mean_time_Systematic]
filters = ['GMF', 'PMF', 'Sys', 'Res', 'Strat']
...
fig.savefig("TIME_HIGH_HILLY.pdf", format='pdf', dpi=1000, bbox_inches='tight')
