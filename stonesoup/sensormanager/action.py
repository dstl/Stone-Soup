...
class ActionGenerator(Base):
    ...
    @property
    def ndim(self):
        """The dimensions of the actions generated by the action generator."""
        raise NotImplementedError()
...
class StateVectorActionGenerator(ActionGenerator):
    ...
    @property
    def ndim(self):
        """Dimensions of an ActionGenerator match the dimension of the actions it produces."""
        return self.initial_value.shape[0]
...
class Actionable(Base, ABC):
    ...
    def add_actions(self, actions: Sequence[Action]) -> bool:
        ...
        if any(action is not None and action.end_time < self.timestamp for action in actions):
