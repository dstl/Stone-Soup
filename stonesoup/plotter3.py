...
class Plotter:
    """3D plotting class for building 3D visualizations of Stone Soup simulations

    A plotting class which is used to simplify the process of plotting ground truths,
    measurements, clutter and tracks with information in three spatial dimensions.
    Tracks can be plotted with uncertainty bars at specified intervals if required.
    ...
    def plot_measurements(self, measurements, mapping, measurement_model=None,
        ...
        for state in measurements_set:
            ...
            if isinstance(meas_model, LinearModel):  # meas_model is a LinearModel type
        ...
        if plot_detections:
            ...
            self.ax.scatter(detection_array[:, 0],
                            detection_array[:, 1],
                            detection_array[:, 2], **measurement_kwargs)
        ...
        if plot_clutter:
            ...
            self.ax.scatter(clutter_array[:, 0],
                            clutter_array[:, 1],
                            clutter_array[:, 2], color='y', marker='2')
    ...
    def plot_tracks(self, tracks, mapping, uncertainty=False, track_label="Track", err_freq=1,
                    **kwargs):
        """Plots track(s)

        Plots each track generated, generating a legend automatically. If ``uncertainty=True``,
        uncertainty bars are plotted every :attr:`err_freq` measurement, default
