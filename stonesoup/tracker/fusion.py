...
class SimpleFusionTracker(Tracker):
    ...
    tracker: Tracker = Property(doc="Tracker given to the fusion node")
    track_fusion_tracker: Tracker = Property(
        doc="Tracker for associating tracks at the node")
    sliding_window = Property(doc="The number of time steps before the result is fixed")
    data = Property(doc="data received from queue and sensor")
    current_time: datetime.datetime = Property(doc='Current time in simulation')
    ...
    def __next__(self):
        ...
        dummy_detector = DummyDetector(current=[time, ])
