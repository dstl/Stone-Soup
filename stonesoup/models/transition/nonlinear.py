...
class DecayTransition(TransitionModel):
    ...
    def function(self, state,  time_interval):
        ...
        undecay_positions = (
            np.random.uniform(size=out_state_vector.shape)
            > self.prob_decay(state, time_interval)
        )
        out_state_vector *= undecay_positions
    ...
    def pdf(self, state_fin, state_ini, time_interval):
        ...
        """
        prob = np.ones(shape=state_ini.state_vector.shape)
