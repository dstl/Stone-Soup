...
from stonesoup.models.transition.linear import \
    (ConstantAccelerationDiscreteTime, CombinedLinearGaussianTransitionModel)


def test_cadt1dmodel():
    """ ConstantAccelerationDiscreteTime1D Transition Model test """
    state_vec = sp.array([[3.0], [1.0], [0.1]])
    noise_diff_coeffs = sp.array([[0.01]])
    base(state_vec, noise_diff_coeffs)


def test_cadt2dmodel():
    """ ConstantAccelerationDiscreteTime2D Transition Model test """
    state_vec = sp.array([[3.0], [1.0], [0.1],
                          [2.0], [2.0], [0.2]])
...
def test_cadt3dmodel():
    """ ConstantAccelerationDiscreteTime3D Transition Model test """
    state_vec = sp.array([[3.0], [1.0], [0.1],
                          [2.0], [2.0], [0.2],
                          [4.0], [0.5], [0.05]])
...
def base(state_vec, noise_diff_coeffs):
    """ Base test for n-dimensional ConstantAccelerationDiscreteTime Transition
     Models """

    # Create a 1D ConstantAccelerationDiscreteTime or an n-dimensional
    # CombinedLinearGaussianTransitionModel object
    dim = len(state_vec) // 3  # pos, vel, acc for each dimension
    if dim == 1:
        model_obj = ConstantAccelerationDiscreteTime(
            noise_diff_coeff=noise_diff_coeffs[0])
    else:
        model_list = [ConstantAccelerationDiscreteTime(
    ...
    base_mat = sp.array(
                    [[1, timediff, sp.power(timediff, 2)],
                     [0, 1, timediff],
                     [0, 0, 1]])
