...
from stonesoup.models.transition.linear import \
    (ConstantVelocityDiscreteTime, CombinedLinearGaussianTransitionModel)


def test_cvdt1dmodel():
    """ ConstantVelocityDiscreteTime1D Transition Model test """
    state_vec = sp.array([[3.0], [1.0]])
    noise_diff_coeffs = sp.array([[0.1]])
    base(state_vec, noise_diff_coeffs)


def test_cvdt2dmodel():
    """ ConstantVelocityDiscreteTime2D Transition Model test """
    state_vec = sp.array([[3.0], [1.0],
                          [2.0], [2.0]])
    noise_diff_coeffs = sp.array([[0.1], [0.2]])
    base(state_vec, noise_diff_coeffs)


def test_cvdt3dmodel():
    """ ConstantVelocityDiscreteTime3D Transition Model test """
    state_vec = sp.array([[3.0], [1.0],
                          [2.0], [2.0],
                          [4.0], [0.5]])
    noise_diff_coeffs = sp.array([[0.1], [0.2], [0.05]])
    base(state_vec, noise_diff_coeffs)


def base(state_vec, noise_diff_coeffs):
    """ Base test for n-dimensional ConstantVelocityDiscreteTime Transition
     Models """

    # Create a 1D ConstantVelocityDiscreteTime or an n-dimensional
    # CombinedLinearGaussianTransitionModel object
    dim = len(state_vec) // 2  # pos, vel, acc for each dimension
    if dim == 1:
        model_obj = ConstantVelocityDiscreteTime(
            noise_diff_coeff=noise_diff_coeffs[0])
    else:
        model_list = [ConstantVelocityDiscreteTime(
    ...
    noise_diff_coeffs = noise_diff_coeffs  # m/s^2
    base_mat = sp.array([[1, timediff],
                         [0, 1]])
    mat_list = [base_mat for num in range(0, dim)]
    F = sp.linalg.block_diag(*mat_list)

    base_covar = sp.array([[sp.power(timediff, 4) / 4,
                           sp.power(timediff, 3) / 2],
                          [sp.power(timediff, 3) / 2,
                           sp.power(timediff, 2)]])
