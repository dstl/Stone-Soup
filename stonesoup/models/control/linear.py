...
class BaseLinearControlModel(ControlModel, LinearModel, GaussianModel, ABC):
...
class LinearControlModel(BaseLinearControlModel):
...
class TransitionBasedLinearControlModel(BaseLinearControlModel):
    ...
    """
    transition_model: TransitionModel = Property(doc="Transition model to apply")
    mapping: Sequence[int] = Property(doc="Mapping to Nth Derivatives of transition model")
    ...
    def matrix(self, time_interval, **kwargs) -> np.ndarray:
        ...
        : :class:`numpy.ndarray`
            the control-input model matrix, :math:`B_k`
        """
        permutation_matrix = identity = np.eye(self.transition_model.ndim)
        if not self.output_in_state_vector:
