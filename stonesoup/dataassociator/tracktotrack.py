    ...
    def associate_tracks(self, tracks_set_1, tracks_set_2):
        ...
        for track2 in tracks_set_2:
            for track1 in tracks_set_1:
                ...
                track1_states = [state for state in track1.states if state.timestamp in truth_timestamps]
                track_timestamps = [state.timestamp for state in track1_states]
                track2_states = [state for state in track2.states if state.timestamp in track_timestamps]
                ...
                if track2_states!=[] and track1_states!=[]:
                    ...
                    distances = [np.linalg.norm(
                        self.measurement_matrix_track1 @ track1_states[i].state_vector.__array__()
                        - self.measurement_matrix_track2 @ track2_states[i].state_vector.__array__())
                    ...
                    for i in range(len(track1_states)):
                        ...
                        if distances[i] <= self.association_threshold:
                            ...
                            if n_succesful >= self.consec_pairs_confirm and not start_timestamp:
                                start_timestamp = track1_states[i-(self.consec_misses_end-1)].timestamp
                            ...
                            if n_unsuccesful >= self.consec_misses_end and start_timestamp:
                                ...
                                end_timestamp = track1_states[i-self.consec_misses_end-1].timestamp
        ...
        for track in tracks_set:
            ...
            for track_state in track.states:
                ...
                for truth in truth_set:
                    ...
                    # If there's a truth state at that timestamp pull it
#                    try:
#                        truth_state = truth[track_state.timestamp]
#                    except:
#                        truth_state = None
                    if track_state.timestamp in [i.timestamp for i in truth.states]:
                        truth_state = truth[track_state.timestamp]
                    else:
                        truth_state = None
        ...
        return AssociationSet(associations)
            ...
            # n_different_to_current = 0
            # n_same_as_current = 0
            # n_sequential = 0
            #
            # for ctruth in closest_truths:
            #
            #     if not current_truth:
        ...
        """
            grouped_ctruth = [(k, sum(1 for i in g)) for k,g in groupby(closest_truths)]
            # Returns a list of tuples of object and number of consecutive occurnaces in the list
            # e.g [(None,2), (<OBJECT A>,10), (None,5), (<OBJECT_B>,8)]

            i_track_state = 0 #Which index of track.states is the beginning of the current group
            current_truth = None
            current_start_timestamp = None
            current_end_timestamp = None
            n_different_to_current = 0
            for group in grouped_ctruth:

                # If truth is the current one being associated to, carry one
                if group[0] == current_truth:
                    continue
                # If no current truth see if this set is large enough to start an association
                elif not current_truth and group[1] >= self.consec_pairs_confirm:
                    current_truth = group[0]
                    current_start_timestamp = track.
                    
                    
                    
                i_track_state += group[0]
        """
