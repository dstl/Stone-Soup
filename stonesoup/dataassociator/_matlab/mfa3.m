function [bestHyp, hypsOut] = mfa3(costs, trackIDs, measHistories, slideWindow)
% measHistories
for hypidx = 1:length(costs)
    hypotheses(hypidx,1).cost = costs(hypidx);
    hypotheses(hypidx,1).trackID = trackIDs(hypidx);
    hypotheses(hypidx,1).measHistory = int64(measHistories(:,:,hypidx));
end
...
% Run MFA
[bestHypotheses, hypothesesOut] = dataAssocPRH([hypotheses; dummyHypotheses], slideWindow);
...
bestHyp.cost = [bestHypotheses.cost];
bestHyp.trackID = [bestHypotheses.trackID];
bestHyp.measHistory = int32(getMeasHist(bestHypotheses, size(bestHypotheses(1).measHistory,1)))';

hypsOut.cost = [hypothesesOut.cost];
hypsOut.trackID = [hypothesesOut.trackID];
hypsOut.measHistory = int32(getMeasHist(hypothesesOut, size(bestHypotheses(1).measHistory,1)))';