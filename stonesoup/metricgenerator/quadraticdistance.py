...
class QuadraticDistance(MetricGenerator):
    ...
    Class for computing the quadratic error between a time series of multi-target state
    representations.

    The quadratic distance is implemented for the Gaussian kernel parametrisation.
    Comparisons can be made between point sets, point sets and Gaussian mixtures and between
    Gaussian mixtures.
    ...
    def quadratic_dist_pp(self, trth, trck, R):
        """
        Method for computing the quadratic distance between two point set states.
        ...
        trth_means = np.array(
            [np.asarray(getattr(trth[x], 'mean', trth[x].state_vector)) for x in range(Phi)])
        trck_means = np.array(
            [np.asarray(getattr(trck[x], 'mean', trck[x].state_vector)) for x in range(Psi)])

        term1 = 0
        term2 = 0
        term3 = 0
        if Phi > 0:
            term1 = self.vectorised_gaussian_eval(
                ret_sum=True,
                normalise=False,
                dim=self.state_dim,
                w1=trth_weights,
                ...
                const_cov=R)
        if Phi > 0 and Psi > 0:
            term2 = self.vectorised_gaussian_eval(
                ret_sum=True,
                normalise=False,
                dim=self.state_dim,
                w1=trth_weights,
                ...
                const_cov=R)
        if Psi > 0:
            term3 = self.vectorised_gaussian_eval(
                ret_sum=True,
                normalise=False,
                dim=self.state_dim,
                w1=trck_weights,
    ...
    def quadratic_dist_pt(self, trth, trck, R):
        """
        Method for computing the quadratic distance between a point set state and a Gaussian
        mixture state.
        ...
        if Phi > 0 and Psi > 0:
            term2 = np.sqrt(
                (2 * np.pi)**self.state_dim * np.linalg.det(R)) * self.vectorised_gaussian_eval(
                ret_sum=True,
                normalise=True,
                dim=self.state_dim,
                w1=trth_weights,
    ...
    def quadratic_dist_tt(self, trth, trck, R):
        """
        Method for computing the quadratic distance between two Gaussian mixture states.
    ...
    def compute_over_time(self, measured_states, truth_states):
        """
        Computes the quadratic distance over time between the measured and true states
    ...
    def vectorised_gaussian_eval(self, ret_sum, normalise, **kwargs):
        ...
        cov = (const_cov + var_cov1[None, :, :, :]
               + var_cov2[:, None, :, :]).reshape(-1, dim, dim)
    ...
    def compute_Q_distance(self, truth_states, track_states):
        ...
        Parameters
        ----------
        truth_states: a list of states. these states must have a state_vector
        attribute but may also have a covar attribute.

        track_states: a list of states. these states must have a state_vector
        attribute but may also have a covar attribute.
        ...
        if self.kernel == 'Gaussian':
            ...
            if ((hasattr(self.truths_type, 'state_vector') and
                 not hasattr(self.truths_type, 'covar')) and
                (hasattr(self.tracks_type, 'state_vector') and
                 not hasattr(self.tracks_type, 'covar'))):

                distance = self.quadratic_dist_pp(truth_states, track_states, R)
            ...
            # quadratic error between track sets
                ...
                distance = self.quadratic_dist_tt(truth_states, track_states, R)
...
class MeanQuadraticError(QuadraticDistance):
    """
    Mean quadratic error of an estimator. the implementation is different for each choice of
    random object as the covariance is different for each point process parametrisation.
    the default case is where we compute the mqe between the posterior point process and
    the truth. in this case, we need the posterior intensity (Gaussian mixture) and the
    truths (Dirac mixture).
    ...
    def kernel_smoothed_covariance(self, estimator_states, hypotheses, updater,
                                   detection_probability, survival_probability, clutter_rate, R):
        '''
        Computes the covariance term of the covariance bias-decomposition of the
        mean quadratic error.

        This term is the inner product of the point process covariance kernel and the kernel
        of the MQE.
        ...
        for z, multi_hypothesis in enumerate(hypotheses[:-1]):
            for i in range(len(multi_hypothesis)):
                for j in range(len(multi_hypothesis)):
                    detection_sum += (update_weights[z][i] * update_weights[z][j] *
                                      multivariate_normal.pdf(np.array(
                                          update_means[z][i]).flatten(), np.array(
                                          update_means[z][j]).flatten(), R + update_covs[z][i] +
                                          update_covs[z][j]))

        mqe_cov_term = update_weights_sum + detection_sum

        return mqe_cov_term

    def compute_MQE(self, parameter_states, estimator_states, hypotheses):
        """
        Method for computing the MQE between a point process (parameter) and an intensity
        (estimator).
        ...
        if self.kernel == 'Gaussian':
            ...
            if self.kernel_parameters is not None:
                ...
                if R.shape != (self.state_dim, self.state_dim) or not np.allclose(
                        R, R.T, rtol=1e-10, atol=1e-10) or np.any(np.linalg.eigvals(R) < 0):
                    raise ValueError(
                        f'The {self.kernel} kernel covariance matrix must be symmetric'
                        f'and positive-definite with shape'
                        f'({self.state_dim}, {self.state_dim}).')
            else:
                raise ValueError(
                    f'No covariance matrix was provided for the' 
