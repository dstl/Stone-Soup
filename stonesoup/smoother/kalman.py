...
class StochasticIntegrationSmoother(KalmanSmoother):
    ...
    def _smooth_gain(self, state, prediction, time_interval, **kwargs):
        ...
        while N < self.Nmin or all([N < self.Nmax,
                                    (np.linalg.norm(VPxx) > self.Eps)]):
            N += 1
            ...
            xpoints = Sf@SCRSigmaPoints + \
                np.matlib.repmat(efMean, 1, np.size(SCRSigmaPoints, 1))
            ...
            SumRPxx = xpoints_diff @ (fpoints_diff *
                                      np.matlib.repmat(w, nx, 1)).T
