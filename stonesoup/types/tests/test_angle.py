...
scenario3 = ('longitude', {'Class': Longitude, 'func': mod_longitude})
scenario4 = ('latitude', {'Class': Latitude, 'func': mod_latitude})


class TestAngleDegrees:
    scenarios = [scenario3, scenario4]
    # In the following functions:
    # Class - this is the class type we use for casting e.g. Longitude
    # func - this is the modulo function

    def test_longitude_init(self, Class, func):
        b1 = Class(180)
        b2 = Class(700)
        assert approx(b1) == func(180)
        assert approx(b2) == func(-20)
        assert b1 == b1
        assert b1 == b1
        assert b1 != b2
