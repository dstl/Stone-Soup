...
class KernelParticleState(State):
    ...
    state_vector: StateVectors = Property(doc="State vectors.")
    weight: np.ndarray = Property(
        default=None, doc="Weights of particles. Defaults to [1/N]*N."
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.kernel_covar is None:
            self.kernel_covar = CovarianceMatrix(
                np.identity(self.state_vector.shape[1]) * (1 / self.state_vector.shape[1])
            )
