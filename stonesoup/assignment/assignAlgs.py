...
def assign2D(C, maximize=False):
    ...
    #
    # INPUTS: C A numRowXnumCol 2D numpy array or matrix matrix that does not
    #          contain any NaNs. Forbidden assignments can be given costs of
    #          +Inf for minimization and -Inf for maximization.
    # maximize A boolean value. If true, the minimization problem is
    #          transformed into a maximization problem.
    #          The default if this parameter is omitted or an empty matrix
    #          is passed is false.
    #
    # OUTPUTS: gain The sum of the values of the assigned elements in C. If
    #               the problem is infeasible, this is -1.
    #       col4row A length numRow numpy array where the entry in each
    #               element is an assignment of the element in that row to a
    #               column. 0 entries signify unassigned rows. If the problem
    #               is infeasible, this is an empty matrix.
    #       row4col A length numCol numpy array where the entry in each element
    #               is an assignment of the element in that column to a row. 0
    ...
    # assigned to one column; otherwise every column is assigned to one row.
    # The assignment minimizes the sum of the assigned elements (the gain).
    # During minimization, assignments can be forbidden by placing Inf in
    # elements. During maximization, assignment can be forbidden by placing
    # -Inf in elements. The cost matrix can not contain any -Inf elements
    # during minimization nor any +Inf elements during maximization to try
    # to force an assignment. If no complete assignment can be made with
    # finite cost, then gain, col4row, and row4col are returned as
    # numpy.empty(0) values.
    #
    ...
    # [3] Murty, K. G. "An algorithm for ranking all the assignments in order
    #    of increasing cost," Operations Research, vol. 16, no. 3,
    #    pp. 682-687, May-Jun. 1968.
    #
    # May 2018 David F. Crouse, Naval Research Laboratory, Washington D.C.
    # (UNCLASSIFIED) DISTRIBUTION STATEMENT A. Approved for public release.
    # This work was supported by the Office of Naval Research through the Naval
