...
class RAGPlotterly(Plotterly):
    ...
    def generate_metrics(tracks: set[Track], truths: set[GroundTruthPath], associator: Associator,
                         metric: MetricGenerator, **kwargs) -> dict:
        ...
        """
        associations = associator.associate_tracks(tracks, truths)
        metric_generators = []
        for track, association in product(tracks, associations):
    ...
    def get_rag_from_value(self, values: list[float], target_value: float, rag_boundaries: RAG
                           ) -> int:
        ...
        """
        value = min(abs(values - target_value))
        if value <= rag_boundaries.GREEN:
            return 3
        if value <= rag_boundaries.AMBER:
            return 2
        return 1
    ...
    def plot_tracks(self, tracks: set[Track], mapping: tuple, metrics: dict, metric_name: str,
        ...
        for track in tracks:
            ...
            for metric_states in zip(*metric_sets):
                values = np.array([metric_state.value for metric_state in metric_states])
                times = {metric_state.timestamp for metric_state in metric_states}
                assert len(times) == 1
                time = next(iter(times))
                colour_dict[time] = self.get_rag_from_value(values, target_value, rag_boundaries)
