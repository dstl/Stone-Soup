...
class SSKMLTrack(object):
	def __init__(self, parent_folder, track_id, track_color, tks_position_array, tks_timestamps=None):
		...
		if tks_timestamps:
			...
			self.track_folder.timespan.begin = time_stamps_start.strftime('%Y-%m-%dT%H:%M:%SZ')
			self.track_folder.timespan.end = time_stamps_end.strftime('%Y-%m-%dT%H:%M:%SZ')
		...
		if tks_timestamps and (len(tks_timestamps) == len(tks_position_array)):
			for track_position, timestamp in zip(tks_position_array, tks_timestamps):
				...
				self.appendSSPointPlacemark(track_position[0], track_position[1],\
					track_position[2],track_color, timestamp)
		else:
			for track_position in tks_position_array:
				track_position = track_position.ravel()
				# Add point placemark
				self.appendSSPointPlacemark(track_position[0], track_position[1],\
					track_position[2], style_id)
	...
	def appendSSPointPlacemark(self, lat, lon, alt, track_color, timestamp=None,\
		...
		if (timestamp):
			point_placemark.timestamp.when = timestamp.strftime('%Y-%m-%dT%H:%M:%SZ')
	...
	def appendSSLinePlacemark(self, lla_ordered_list, track_color, altitude_mode=AltitudeModes.relativeToGround):
		...
		line_placemark.style.linestyle.width = settings.TRACK_LINE_WIDTH 
...
class SSKML(object):
	...
	def __init__(self):
		...
		self._doc.name = settings.DOC_NAME
		self._tks_style_idx = 0
		self._det_style_idx = 0
		self._tks_max_colors = 128
		self._det_max_colors = 128
		self._tks_cmap_norm = matplotlib.colors.Normalize(vmin=1, vmax=self._tks_max_colors)
		self._det_cmap_norm = matplotlib.colors.Normalize(vmin=1, vmax=self._det_max_colors)
	...
	def appendTracks(self, tks_position_matrix, tks_ids, tks_timestamp_matrix=None):
		tks_folder = self._doc.newfolder(name=settings.TRACKS_FOLDER_NAME)
		...
		if (tks_timestamp_matrix):
			for track_positions, track_id, track_timestamps in zip(tks_position_matrix, tks_ids, tks_timestamp_matrix):
				track_color_str = self._getTrackColor()
				SSKMLTrack(tks_folder, track_id, track_color_str, track_positions, track_timestamps)
		else:
			for track_positions, track_id in zip(tks_position_matrix, tks_ids):
				track_color_str = self._getTrackColor()
				SSKMLTrack(tks_folder, track_id, track_color_str, track_positions)
				



	def appendDetections(self, det_position_array, det_time_array=None):
		detections_folder = self._doc.newfolder(name=settings.DETECTIONS_FOLDER_NAME)
		...
		if (det_time_array is not None):
			...
			for det_position, det_timestamp in zip(det_position_array, det_time_array):
				...
				det_color_str = self._getDetColor()
				det_folder = detections_folder.newfolder(name="Detection_{}".format(k))
				det_placemark = det_folder.newpoint()
				det_placemark.timestamp.when = det_timestamp.strftime('%Y-%m-%dT%H:%M:%SZ')
