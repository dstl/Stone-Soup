...
class NMEAReader(DetectionReader):
    """
    A simple detection reader for NMEA log files of AIS (maritime
    transponder) detections.

    Currently, this class only parses message types 1,2, and 3, which represent
    Class A AIS Position reports.
    ...
    * The AIS detection attributes for lattitude, longitude, and timestamp are
    ...
    For more information on NMEA and AIS visit:

    [1] https://www.navcen.uscg.gov/?pageName=AISMessages
    ...
    bbox = Property(
        ...
        messages from vessels within the bounding box are read as detections.\
        If this is not given then all messages are read. Format: \
        left, bottom, right, top in decimal degrees (latitude/longitude).")
    start_time = Property(
        datetime,
    ...
    def __init__(self, *args, **kwargs):
        ...
        time_span = [int((ktime - self.reference_utc).total_seconds())
                     for ktime in target_time]
    ...
    def load_ais_types(self):
        ...
        try:
            ...
            print("Unable to parse AIS types definition\
             file at {}".format(self.ais_types_defs))
            raise Exception(e)

        ais_types = [[AISField(field) for field in ais_type]
                     for ais_type in types_list]
        return ais_types
    ...
    def detections_gen(self):
        ...
        for record in self._aLog:
            ...
            try:
                ais_dict = {fld.name: self.decoder.decodeField(fld)
                            for fld in ais_types[msg_type - 1]}
            except IndexError:
                # Undefined messages (message type > 27)
            ...
            if (msg_type < 4):
                ...
                if (('Latitude' in ais_dict) and ('Longitude' in ais_dict)):
                    coords = [float(ais_dict['Latitude'][0]),
                              float(ais_dict['Longitude'][0])]
                    # Check if the coordinate is inside the polygon.
                    if (self.bbox_contains(coords[0], coords[1])):
                        if ((utc_time >= self.start_time) and
                           (utc_time <= self.end_time)):
                            # Append to detections.
                            msg_meta = {'mmsi': ais_dict['MMSI'][0]}
                            det = Detection(StateVector([[coords[1]],
                                            [coords[0]], [0.0]]),
                                            timestamp=utc_time,
                                            metadata=msg_meta)
                    ...
                    # Message type 1,2, or 3 found without latitude and
                    # longitude. Skipping.
                    continue
            else:
