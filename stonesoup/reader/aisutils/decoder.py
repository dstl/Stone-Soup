...
class NMEADecoder(object):
	ais_six_bit_text = "@ABCDEFGHIJKLMNOPQRSTUVWXYZ[\]^_ !\"#$%&\\()*+,-./0123456789:;<=>?"
	payload = ""
	decode_dictionary = {}
	...
	def joinFields(self, source, masks, shifts):
		...
		and shifts.
		"""
		six_bit = [(ord(x)-48 -int(ord(x)>88)*8)&y for x,y in zip(source, masks)]
		try:
			return functools.reduce(lambda x,y: x|y,
				[x<<y for x,y in zip(six_bit, shifts) if y>=0]
				+ [x>>abs(y) for x,y in zip(six_bit, shifts) if y<0]
				)
		except TypeError:
