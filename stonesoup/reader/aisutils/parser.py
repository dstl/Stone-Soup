...
class NMEA(object):
	"""
	Generic class for NMEA message log files. 
	Do not instantiate from this class. Instead, use one
	of the sub-classes.
	"""
	...
	def next(self):
		...
		if (self.time_span[-1] > 0 and int(target_line[-1]) > self.time_span[-1]):
			raise StopIteration
		# Update the count
		self.updateCount()
	...
	def checkSum(self, src):
		...
		if (m):
			...
			return functools.reduce(lambda x,y:x^y, [ord(x) for x in chksum_bdy])
		return 0xFF
	...
	def advanceToTime(self, t_advance):
		...
		if (last_line is None):
			...
			if (t_advance < time_span[0] or t_advance > time_span[-1]):
				...
				fraction = float(t_advance - time_span[0])/(time_span[-1] - time_span[0])
