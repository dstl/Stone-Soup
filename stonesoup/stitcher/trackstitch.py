...
class TrackStitcher(Stitcher):
    r"""
        Track Stitcher has only been tested using:
            TransitionModel: OrnsteinUhlenbeck, ConstantVelocity
            Predictor: Kalman
            Updater: Kalman
            Hypothesiser: Distance(Mahalanobis)
        PLEASE NOTE: Backward Predictor will only work when the KalmanBackwardPredictor has been added to Stone Soup
        """
    forward_hypothesiser: Hypothesiser = Property(
        default=None,
        doc="This is the Hypothesiser used for forward prediction in the track stitcher")
    backward_hypothesiser: Hypothesiser = Property(
        default=None,
        doc="This is the Hypothesiser used for backward prediction in the track stitcher")
    ...
    def stitch(self, tracks, start_time):
        ...
        if self.forward_hypothesiser is None and self.backward_hypothesiser is None:
            print("Please pass in at least 1 hypothesiser")
            return
