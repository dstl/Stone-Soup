...
class NUTSProposal(Proposal):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.lag_size:
            self.blender_func = self._blend_array
            self.leap_frog_func = self.integrate_lf_vec_array
        else:
            self.blender_func = self._blend_state
            self.leap_frog_func = self.integrate_lf_vec
    ...
    def merge_states_dir(self, xminus, vminus, grad_xminus, xplus, vplus,
        ...
        if self.lag_size:
            mask = direction < 0
            merge_state = self.blender_func(xminus, xplus, mask)
            ...
            mask = direction < 0
            xmerge = self.blender_func(xminus, xplus, mask).state_vector
            vmerge = self._blend_array(vminus, vplus, mask)
            grad_xmerge = self._blend_array(grad_xminus, grad_xplus, mask)
    ...
    def rvs(self, state, measurement: Detection = None,
        ...
        if not self.lag_size:
            # Standard nuts approach
            ...
            if measurement is not None:
                ...
                weights_update = self._update_weights(previous_state, new_nuts_state, measurement,
                                                      time_interval, v, v_new, grad_x)
            ...
            final_state.log_weight += weights_update
            return final_state

        else:
            # fix lag nuts approach
