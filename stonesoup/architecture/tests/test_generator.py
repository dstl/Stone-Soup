...
from stonesoup.architecture import Architecture
from stonesoup.architecture.edge import FusionQueue, Edges
...
def test_info_generate_hierarchical(generator_params):
    ...
    for arch in archs:
        ...
        assert len(arch.edges) == sum([3, 1, 1]) - 1
...
def test_info_generate_decentralised(generator_params):
    ...
    for arch in archs:
        ...
        assert len(arch.edges) == sum([3, 1, 1])
...
def test_invalid_graph(generator_params):
    start_time = generator_params['start_time']
    base_sensor = generator_params['base_sensor']
    base_tracker = generator_params['base_tracker']
    ...
    gen = NetworkArchitectureGenerator(arch_type='decentralised',
                                       ...
                                       base_sensor=base_sensor,
                                       n_archs=3,
                                       allow_invalid_graph=False)

    with pytest.raises(ValueError):
        gen.generate()

    gen2 = NetworkArchitectureGenerator(arch_type='decentralised',
                                        start_time=start_time,
                                        mean_degree=0.5,
                                        node_ratio=[3, 1, 1],
                                        base_tracker=base_tracker,
                                        base_sensor=base_sensor,
                                        n_archs=3,
                                        allow_invalid_graph=True)
    archs = gen2.generate()

    for arch in archs:
        assert isinstance(arch, Architecture)
        assert isinstance(arch.edges, Edges)
