...
def tracking_DGP( T, time_data, x_train, y_train):
    ...
    for i in range(N, T):
        ...
        for sensor_id in range(len(y_train)):
            ...
            x_noise = np.random.normal(0.1, 1, x_filtered.shape)  # Replace mu and sigma with your chosen values
            x_filtered += x_noise
            ...
            y_filtered=(np.array(y_train[sensor_id])[
                                   indices_to_keep].reshape(-1, 1))
            y_noise = np.random.normal(0.1, 1, y_filtered.shape)  # Replace mu and sigma with your chosen values
            y_filtered += y_noise
...
seed =113; num_sensors = 8
min_distance = 250; range_value = 350
xrange = (2200, 4200)
yrange = (-250, 250)
...
for experiment in range(EN):
    ...
    xd_data, xd_cov, yd_data, yd_cov = tracking_DGP( T, time_data1, x_data1, y_data1)
    ...
    # # 存储每次实验的轨迹数据
    # all_true_X.append(true_X1)
    # all_true_Y.append(true_Y1)
    # all_estimated_X.append(xd_data)
    # all_estimated_Y.append(yd_data)
...
# Plot the data
ax2.plot(true_X1, 'd-k', label='True Trajectory', markerfacecolor='None', markersize=5)
...
# Plot the data
ax2.plot(true_Y1, 'd-k', label='True Trajectory', markerfacecolor='None', markersize=8)
...
ax1.plot(true_Y1, 'd-k', label='True Trajectory', markerfacecolor='None', markersize=8)
...
# Plotting the data over time
ax1.plot(true_X1, 'd-k', label='True Trajectory', markerfacecolor='None', markersize=8)
