...
# ----------------------------------
# Angular state space data is generated by measuring the previously generated cartesian state space
# with a :class:`~.RadarElevationBearingRange` sensor:
...
# %%
# Each ground truth is measured individually and without noise to create ground truth paths:
...
# %%
# Bearing (Radians) vs Time (s)
# ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
mapping = [0]
plotter_az_t_cart = Plotterly(title="Cartesian - Bearing (Radians) vs Time (s)",
...
# %%
# Bearing (Degrees) vs Time (s)
# ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
mapping = [0]
plotter_az_t = PolarPlotterly(title="Polar - Bearing (Degrees) vs Time (s)")
...
# %%
# Bearing (Degrees) vs Range (m)
# ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
plotter_az_r = PolarPlotterly(title="Bearing (Degrees) vs Range (m)")
mapping = [0, 1]
...
# ^^^^^^^^^^^^^^^^^
# For comparison, this is the previous x-y plot with the detections displayed.
