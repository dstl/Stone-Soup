...
# %%
# The Probabilistic Multi-Hypothesis Tracker (PMHT) is an advanced tracking algorithm designed to handle multiple
# hypotheses in a tracking scenario. Originally introduced by Streit and Luginbuhl [#]_, the PMHT is particularly
# useful when dealing with complex scenarios. Such scenarios include the presence of multiple potential targets and
# uncertainties in measurements.
#
# In PMHT, the objective is to track multiple targets evolving independently over time. Each target's movement is
# modeled probabilistically, typically using a Markov transition model. In each time step, received measurements can
# correspond to either one of the target models or clutter. Clutter refers to measurements that do not correspond to
# any actual target.
#
# One of the key challenges in multi-target tracking is data association, i.e., correctly associating measurements with 
# their respective targets. PMHT addresses this by considering multiple hypotheses for  data association.
...
#
# We denote by :math:`X` the joint states of all targets over time steps 1 to T, and  similarly let :math:`Z`
# and :math:`K` denote the joint measurement and association variables. Unlike data association algorithms, such as the
# JPDA Filter, the PMHT ignores the constraint that each target can generate at most
# one detection. This simplification allows us to write down the joint probability as:
#
# .. math::
#          p(Z, X, K | \Pi) = \left(\prod_{m=1}^M \phi^m_0(x^m_0)\right)\prod_{t=1}^T\left\{ \left(\prod_{m=1}^M
#          \phi^m_t(x^m_t | x^m_{t-1}) \right) \left(\prod_{r=1}^{n_t}\pi_t^{k^t_r}
...
# to target :math:`m` (or clutter if :math:`m=0`), and :math:`\Pi` is the joint variable over all target hypotheses
# and time steps. From this, the algorithm proceeds in a manner similar to the Expectation-Maximisation algorithm.
# Given an initial estimate :math:`X` of the target states and :math:`\Pi` of the association probabilities, we choose
# new values :math:`X` and :math:`\Pi` which maximise
#
# .. math::
...
# of a configurable number of time steps and predict forward using the prior means.

# %%
# Nearly-constant velocity example
# --------------------------------
# In a similar fashion to the previous tutorials, we begin with some imports. These ones allow us access to
...
# number of configurable parameters, e.g. where, at what rate or detection probability a clutter is generated.
# This implements similar logic to the code in the previous tutorial section
# :ref:`auto_tutorials/09_Initiators_&_Deleters:Generate Detections and Clutter`. Note that, currently, very low
# clutter rate is used as PMHT seems to struggle with high clutter rate.
...
# tracks, with data association built in. Therefore, the usage of nitiator or deleter component are optional best
...
# Run the Tracker
# ---------------
# The PMHT algorithm is a batch-based approach that processes multiple scans simultaneously. It uses a set of scans
# from  the previous batch to initialize the new batch for improved performance. The algorithm runs a set number
# of iterations per batch for convergence, with a maximum iteration limit. Though a convergence test might stop
...
# Key points
# ----------
# 1. PMHT uses a probabilistic framework and Bayesian recursion to update beliefs about target states.
...
#        Report, Naval Undersea Warfare Division, 1995 problems, IEE Proc., Radar Sonar Navigation, 146:2–7
