...
# %%
# The Probabilistic Multi-Hypothesis Tracker (PMHT) is an advanced tracking algorithm designed to handle multiple hypotheses
# in a tracking scenario. Originally introduced by Streit and Luginbuhl [#]_, the PMHT is particularly useful when dealing with
# complex scenarios where there are multiple potential targets and uncertainties in measurements.
#
# In PMHT, the objective is to track multiple targets evolving independently over time. Each target's movement is modeled
# probabilistically, typically using a Markov transition model. At each time step, measurements are received, which may 
# correspond to any of the target models or clutter. Clutter refers to measurements that do not correspond to any actual target.
#
# One of the key challenges in multi-target tracking is data association, i.e., correctly associating measurements with 
# their respective targets. PMHT addresses this challenge by considering multiple hypotheses for data association, allowing 
# for the possibility of different associations between measurements and targets.
#
# Let the motion of target :math:`m` from time step :math:`t-1` to :math:`t` be given by the following Markov transition model:
#
# .. math::
#           p(x^m_t | x^m_{t-1}) = \phi^m_t(x^m_t | x^m_{t-1})
#
# Let :math:`k^t_r` be the target which generates measurement :math:`r` at time step :math:`t`. Then the measurement :math:`z^t_r` conditional
# on the assignment is assumed to be given by the following measurement model:
#
# .. math::
#          p(z^t_r | x^{m}_t, k^t_r = m) = \begin{cases} 1/V & \text{if } k^t_r = 0 \\ \zeta(z^t_r | x_m) & \text{otherwise} \end{cases}
#
# We denote by :math:`X` the joint states of all targets over time steps 1 to T, and similarly let :math:`Z` and :math:`K` denote the joint
# measurement and association variables. Unlike data association algorithms such as the Joint Probabilistic Data Association 
# Filter, the PMHT ignores the constraint that each target can generate at most one detection. This simplification allows 
# us to write down the joint probability as:
#
# .. math::
...
#
# where :math:`\pi^m_t` is the prior probability of a measurement at time step :math:`t` being assigned
# to target :math:`m` (or clutter if :math:`m=0`), and :math:`\Pi` is the joint variable over all target hypotheses and time steps.
# From this, the algorithm proceeds in a manner similar to the Expectation-Maximisation algorithm.
# Given an initial estimate :math:`X` of the target states and :math:`\Pi` of the association probabilities, we choose new values :math:`X` and :math:`\Pi` which maximise
#
# .. math::
#         Q(\Pi, X | \Pi^\prime, X^\prime) = \sum_K \left\{\log p(Z, X, K | \Pi)p(K | Z, X^\prime, \Pi^\prime)\right\}
#
# This is repeated over a number of iterations. In this implementation, the prior means are used for the initial
# estimates of the target states for the first batch of measurements. For subsequent batches, we keep an overlap 
# of a configurable number of time steps and predict forward according to the prior means.
#
#
# Nearly-constant velocity example
# --------------------------------
# In a similar fashion to the previous tutorials, we begin with some imports. These ones allow us access to mathematical and timing functions.
#
# Ground truth
# ^^^^^^^^^^^^
...
groundtruth_sim = MultiTargetGroundTruthSimulator(
    ...
    preexisting_states=preexisting_states, # State vectors at time 0 for groundtruths which should exist at the start of simulation.
    ...
    birth_rate=0.0, # Rate at which tracks are born. Expected number of occurrences (λ) in Poisson distribution. Default 1.0.
...
# %%
# And plot them using AnimatedPlotterly
...
# Key points
# ----------
# 1. PMHT employs a probabilistic framework, utilizing Bayesian recursion to iteratively update beliefs about 
#    target states based on measurements, thereby accounting for uncertainties in target motion and measurement noise.
# 2. An essential aspect of PMHT is the association of predicted target states with measurements. This association step, often
#    referred to as data association, involves determining which measurements correspond to which targets. PMHT typically employs 
#    algorithms or methods for hypothesis generation and evaluation to associate measurements with predicted target states effectively.
#
# References
# ----------
# .. [#] R. Streit and T. Luginbuhl, “Maximum likelihood method for probabilistic multihypothesis
#        tracking”, Proceedings of SPIE, 1994.
# .. [#] R. Streit and T. Luginbuhl”, Probabilistic Multi-Hypothesis Tracking”, Technical Report, Naval Undersea Warfare Division, 1995
#        problems, IEE Proc., Radar Sonar Navigation, 146:2–7
# .. [#] S. Davey, D. Gray and R. Streit, “Tracking, association, and classification: a combined PMHT approach”, Digital Signal
#        Processing 12, 372–382, 2002.
