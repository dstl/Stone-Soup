...
# states. There exists an infinite number of functions which may be used for this purpose; a
...
for n, timestep in enumerate(timesteps[1:]):
    ...
    for sensor in sensor_setA:
        ...
        measurementsA |= sensor.measure(
            OrderedSet(
                gt for gt in alive_truths if np.random.rand() <= probability_detection),
            noise=True)
    ...
    for reduced_state in reduced_statesA:
        if reduced_state.weight > 0.05:
            all_gaussiansA[n].append(reduced_state)
...
# the QIG correspond to those which minimise the posterior uncertainty.
...
for n, timestep in enumerate(timesteps[1:]):
    ...
    for reduced_state in reduced_statesB:
        if reduced_state.weight > 0.05:
            all_gaussiansB[n].append(reduced_state)
...
# estimator of the true population state. This assessment of quality will
# take into account the second order moment of the posterior distribution,
# i.e., the uncertainty in the number of targets. If this is large, then
# the estimation quality is deemed to be lower. The quadratic distance is
# also used to compare the performance of the two filters, but in this
# case will only operate on the first order moment of the posterior
# distribution. Using the quadratic distance, we consider the error
# between the truths and the extracted means from the gaussian mixture
...
mquaderr_posterior_truth2 = MeanQuadraticError(filter_data=filter_data_dict,
                                               ...
                                               tracks_key='tracks2',
                                               hypotheses_key='hypotheses2',
                                               truths_key='truths')

metric_manager = MultiManager([quaderr_pp1,
                               quaderr_pt1,
                               quaderr_pp2,
                               quaderr_pt2,
                               quaderr_tt,
                               mquaderr_posterior_truth1,
                               mquaderr_posterior_truth2])
...
# the true multi-target state, i.e., the bias of the estimator. In this case, the posterior of
# the two filters is considered, at each timestep, as an estimator of the multi-target state.
# This estimator, in the case of the GM-PHD filter, takes the form of a point process which is
# constructed as the sum of a Poisson point process and N-Bernoulli point processes. The first
# moment of this point process, the intensity, is propagated by the filter. This intensity is
# given by a Gaussian mixture. The second moment however, is not. However, the second moment
# is considered in the computation of the MQE and hence, the uncertainty of the posterior is
# accounted for in this evaluation. This uncertainty can be interpreted as the uncertainty in
# the estimated number of targets. The bias component of the MQE is computed as the quadratic
# distance between the truth point patterns and the Gaussian mixture intensities. This bias
# computation takes into account the localisation uncertainty encoded in the Gaussian mixture
# by the covariance of each component. The MQE can thus be interpreted as a combination of
...
# not optimally tasked with the goal of collecting observations of relevant targets. The bias of
# this estimator is also expected to be high, as without suitable observations, the filter will
...
# that the chosen sensor configuration is the one which minimises the posterior uncertainty.
# Due to this, we would expect the MQE of this estimator to be lower than that of the filter
# based on randomly managed sensors - even if the accuracy of the filters are both kept constant.
...
# Furthermore the reduction process of the Gaussian mixture intensity introduces further
