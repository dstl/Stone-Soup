...
# %%
# Extended object tracking (EOT) is a research topic that deals with
# tracking algorithms where the dimensions of the target have a non-negligible role in the
# detection generation. Advancements in the resolution capabilities of sensors, such as RADARs and LIDARs,
# allow to collect more signals from the same object at the same instant, or direct detection
# of the shape (or part of) the target. This is important in many research areas such as self-driving
# autovehicles.
#
...
# In this example, we consider the case where we collect multiple detections from the targets,
# the detections are sampled from the an elliptic shape, an approximation of the target extent,
# then we use a simple clustering algorithm to identify the centroid of the distribution that
# will be used for tracking.
#
# This example will consider multiple targets on a nearly constant velocity dynamics,
# a non-negligible clutter level and we do not consider any bounded environments (i.e. roads)
# for the targets and we allow collisions to happen.
#
...
# 1. Describe the targets ground truths;
# --------------------------------------
# We consider three targets moving on a nearly constant velocity dynamics.
# In the ground truth states we include the metadata describing the size and orientation of the
# target.  The target shape is described by the 'lenght' and 'width', semi-major and semi-minor
# axis of the ellipse, and the 'orientation'. The 'orientation' parameter is
# infeered by the state velocity components at each instant, while we assume that the size parameters are kept
...
metadata_tg1 = {'length': 10,
                'width': 5,
                'orientation': np.arctan(target_state1.state_vector[3]/target_state1.state_vector[1])}
...
metadata_tg2 = {'length': 20,
                'width': 10,
                'orientation': np.arctan(target_state2.state_vector[3]/target_state2.state_vector[1])}
...
metadata_tg3 = {'length': 8,
                'width': 3,
                'orientation': np.arctan(target_state3.state_vector[3]/target_state3.state_vector[1])}
...
for itarget in range(len(targets)):
    ...
    for k in range(1, num_steps):  # loop over the timesteps
        new_metadata = {}  # create a new dictionary

        # Copy the old metadata
        new_metadata['length'] = truth[k-1].metadata['length']
        new_metadata['width'] = truth[k-1].metadata['width']

        # Evaluate the new state
        new_state =  transition_model.function(truth[k-1],
                                               noise=True,
                                               time_interval=timedelta(seconds=5))
        # evaluate the new orientation
        new_metadata['orientation'] = np.arctan2(new_state[3], new_state[1])
...
# 2. Collect the measurements from the targets;
# ---------------------------------------------
# We have the trajectories of the targets, we can specify the measurement model. In this
# example we consider a :class:`~.LinearGaussian` measurement model.
# For this application we adopt a different approach from other examples, for each target state we create an
# oriented shape, centred in the ground-truth x-y location, and from it we draw a number of points.
#
# In detail, at each timestep we evaluate the orientation of the ellipse from the velocity state
# of each target, then we randomly select between 1 and 10 points, assuming at least a detection per timestamp.
# The sampling on elliptic distribution is done using an Inverse-Wishart distribution.
# We use such points to generate target detections.
#
# We generate scans which contain both the detections and noise measurements from the
# targets.
...
for k in range(num_steps):
    ...
    for truth in truths:
        ...
        # Identify how many detection obtain
        ...
        rotation = np.array([[np.cos(current.metadata['orientation']), -np.sin(current.metadata['orientation'])],
                             [np.sin(current.metadata['orientation']), np.cos(current.metadata['orientation'])]])
        ...
        # Sample points
        sample_point = np.atleast_2d(multivariate_normal.rvs(mean_centre, covariance_matrix, size=sampling_points))
...
# :class:`~.ExtendedKalmanUpdater` to perform the tracking. We consider a distance based data associator
# using :class:`~.GlobalNearestNeighbour`. We employ a :class:`~.MultiMeasurementInitiator` for initiating
# the tracks and a time based deleter using :class:`~.UpdateTimeStepsDeleter`.
#
# To process the cloud of detections generated at each timestep we use a K-means clustering method.
# This method identifies datapoints closer together (using Euclidean distance measure) as elements
# belonging to the same cluster, identified the clusters we can obtain the centroid distribution that
# will be passed to tracker as unique detection.
#
# This simple example does not consider more refined clustering methods (e.g., inferring the number of
# clusters using the elbow method) or employing a different measurement model (which can include the
# shape of the target), but highlights a way to use the current implementation for EOT purposes.
...
# ^^^^^^^^^^^^^^^^^^^^^^
# Before running the tracker we employ a simple clustering method
# to identify neighbouring detections to the same target and identify a detection centroid, which
# will be used by the tracker.
# In this example, we assume known the number of targets and that number is not changing
# as the simulation evolve. More detailed approaches would solve these assumptions for a more
# general formulation.
...
# Conclusion
# ----------
# In this example, we have presented a way to use Stone Soup components for Extended Object tracking.
# We have shown how to generate multiple detections out of a single target known position, by random
# sampling on an elliptic extent of the target. We have applied a clustering algorithm to
# identify cloud of detections as coming from the same target and use such knowledge to perform the
# tracking.
...
# [#] .. Granstrom, Karl, Marcus Baum, and Stephan Reuter. "Extended object tracking: Introduction,
