...
# consideration, there are false alarms or clutter, targets can appear and disappear. That is to say
# it happens everywhere.
#
# In this tutorial we introduce **global nearest neighbour** data association, which
# attempts to find a globally-consistent collection of hypotheses such that some overall probability
# of correct association is maximised.
#
# Make the assumption that each target generates, at most, one measurement, and that
# one measurement is generated by, at most, a single target, or is a clutter point. Under these assumptions,
# global nearest neighbour will assign measurements to predicted measurements to
# minimise a total (global) cost which is a function of the sum of innovations. This is an example of an
# assignment problem in combinatorial optimisation.
#
...
# ^^^^^^^^^^^^^^^^^^^^^^^^
# With multiple targets to track, the :class:`~.NearestNeighbour` algorithm compiles a list of all hypotheses
# and selects pairings with higher scores first.
#
...
# detection scores higher relative to it. This association leads to a global score of :math:`0.6` - a
# better net score than the :math:`0.51` returned by the nearest neighbout algorithm.
