...
class UncertaintyManager(Base):
    ...
    def calculate_reward(self, tracks_list, config, metric_time):
        ...
        for obs, sensor in zip(set(config), self.sensor_list):
...
# targets,  which are updated. These states are appended to the tracks list.
#
...
for timestep in timesteps[1:]:
    ...
    for sensor in sensor_set:
        ...
        measurementsA.append(measurement.pop())
...
for timestep in timesteps[1:]:
    ...
    sensor_list = uncertaintymanager.sensor_list
    measurementsB = []

    for target, sensor in zip(chosen_targets, sensor_list):
        # The ground truth will therefore be:
        selected_truth = truths[target][timestep]

        # Observe this
        measurement = sensor.measure([selected_truth], noise=True)
        measurementsB.append(measurement.pop())
